...
.PHONY: build install uninstall clean test optimize-images build-docs serve-docs pub-docs help
...
linux windows darwin amd64 arm64 armv6l 386:
	@exit 0
...
%:
	@:
	@if [ -z "$(CMD_STR)" ] || [ "$(CMD_STR)" = "ARGS=" ]; then \
		$(MAKE) help; \
		exit 0; \
	fi

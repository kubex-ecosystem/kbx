...
__get_values_from_manifest() {
  ...
  _VERSION_GO="$(jq -r '.go_version' "$_ROOT_DIR/$_MANIFEST_SUBPATH" 2>/dev/null || grep '^go ' "$_ROOT_DIR/go.mod" | awk '{print $2}' || echo "")"
  _VERSION_GO="${_VERSION_GO//null/}" # Remove 'null' if present

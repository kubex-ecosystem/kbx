...
var (
	TempDir      = os.TempDir()
	poolSize     = 512
	poolChan     = make(chan struct{}, poolSize)
	poolFileChan = make(chan struct{}, poolSize*2)
)

var signalsManager *SignalManager[chan string]

var (
	mapper Mapper[any]
	sigCtl chan string
	// dsClient ds.DSClient
)

func init() {
	sigCtl = make(chan string)
	signalsManager = newSignalManager(sigCtl, gl.GetLoggerZ("glb_init"))
	go signalsManager.ListenForSignals()
}

...
type ValidationFunc[T any] struct {
	Priority int
	Func     func(value *T, args ...any) IValidationResult
	Result   IValidationResult
}

func newValidationFunc[T any](priority int, f func(value *T, args ...any) IValidationResult) *ValidationFunc[T] {
...
func (vf *ValidationFunc[T]) GetFunction() func(value *T, args ...any) IValidationResult {
	if vf == nil {
		return nil
	}
	return vf.Func
}
func (vf *ValidationFunc[T]) SetFunction(f func(value *T, args ...any) IValidationResult) {
	if vf == nil {
		return
	}
	vf.Func = f
}
func (vf *ValidationFunc[T]) GetResult() IValidationResult {
	if vf == nil {
		return nil
	}
	return vf.Result
}
func (vf *ValidationFunc[T]) SetResult(result IValidationResult) {
	if vf == nil {
		return
	}
	vf.Result = result
...
type Validation[T any] struct {
	...
	validateFunc func(value *T, args ...any) IValidationResult
}

// vldtFunc is a function that validates the value.
func vldtFunc[T any](v *Validation[T]) func(value *T, args ...any) IValidationResult {
	return func(value *T, args ...any) IValidationResult {
		if v == nil {
			return newValidationResult(false, "validation is nil", nil, gl.Errorf("validation is nil"))
		}
		if !v.IsValid() {
			return newValidationResult(false, "validation is invalid", nil, gl.Errorf("validation is invalid"))
		}
...
func VldtFunc[T any](v IValidation[T]) func(value *T, args ...any) IValidationResult {
	return func(value *T, args ...any) IValidationResult {
		if v == nil {
			return NewValidationResult(false, "validation is nil", nil, gl.Errorf("validation is nil"))
		}
		...
		for _, arg := range args {
			if validator, ok := arg.(IValidationFunc[T]); ok {
...
func NewValidation[T any]() IValidation[T] {
	validation := &Validation[T]{
		isValid:      false,
		validatorMap: sync.Map{},
	}
...
func (v *Validation[T]) Validate(value *T, args ...any) IValidationResult {
	if v == nil {
		return NewValidationResult(false, "validation is nil", nil, gl.Errorf("validation is nil"))
	}
	if value == nil {
		return NewValidationResult(false, "value is nil", nil, gl.Errorf("value is nil"))
	}
	if !v.hasValidation {
		return NewValidationResult(false, "validation has no validators", nil, gl.Errorf("validation has no validators"))
	}

	v.mu.Lock()
	defer v.mu.Unlock()

	results := make([]IValidationResult, 0)
...
func (v *Validation[T]) AddValidator(validator IValidationFunc[T]) error {
	if v == nil {
		return gl.Errorf("validation is nil")
	}
	...
	if validator.GetFunction() == nil {
		return gl.Errorf("validator function is nil")
	}
	if validator.GetPriority() < 0 {
		return gl.Errorf("priority must be greater than or equal to 0")
	}
	if _, ok := v.validatorMap.LoadOrStore(validator.GetPriority(), validator); ok {
		return gl.Errorf("validator with priority %d already exists", validator.GetPriority())
	}
...
func (v *Validation[T]) RemoveValidator(priority int) error {
	...
	if _, ok := v.validatorMap.LoadAndDelete(priority); !ok {
		return gl.Errorf("validator with priority %d does not exist", priority)
	}
...
func (v *Validation[T]) GetValidator(priority int) (any, error) {
	if v == nil {
		return nil, gl.Errorf("validation is nil")
	}
	if !v.hasValidation {
		return nil, gl.Errorf("validation has no validators")
	}
	if validator, ok := v.validatorMap.Load(priority); ok {
		return validator, nil
	}
	return nil, gl.Errorf("validator with priority %d does not exist", priority)
}

// GetValidators is a function that gets the map of validators.
func (v *Validation[T]) GetValidators() map[int]IValidationFunc[T] {
	if v == nil {
		return nil
	}
	if !v.hasValidation {
	...
	validatorMapSnapshot := make(map[int]IValidationFunc[T])
	v.validatorMap.Range(func(key, value any) bool {
		if validator, ok := value.(IValidationFunc[T]); ok {
...
func (v *Validation[T]) GetResults() map[int]IValidationResult {
	if v == nil {
		return nil
	}
	if !v.hasValidation {
	...
	results := make(map[int]IValidationResult)
	v.validatorMap.Range(func(key, value any) bool {
		if validator, ok := value.(IValidationFunc[T]); ok {
